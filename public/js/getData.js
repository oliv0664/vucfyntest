function getStudentData(studentID, teacherID, index, contentdb) {
    ...
    console.log('test this: ', contentdb);
    ...
    if (content.tests) {
        ...
        console.log(content["Indgår der læsning eller skrivning i dit job"]);
        ...
        var $td371 = $(`
                    ...
                    <td>:</td>
                    <td>` + content["Indgår der læsning eller skrivning i dit job"] + `</td>
                    `);
        var $td372 = $(`
        ...
        for (i = 0; i < content.tests.length; i++) {
            ...
            if (currentTest.type == 'orddiktat') {
                ...
                $th = $('<th>Kursist svar</th><th>tid</th>');
            }
            $tr = $('<tr/>').append($th);
            $table.append($tr);
            var count = 0;
            ...
            for (var j = 0; j < currentTest.answers.length; j++) {
                ...
                } else if (currentTest.type == 'brev') {
                    $td = $(`
                    <td>` + currentTest.answers[j].answer + `</td>
                    <td>` + Math.floor(currentTest.answers[j].time/60000) + ' minutter og ' + (currentTest.answers[j].time % 60000)/1000 + ' sekunder' +  `</td>
                    `);
                }
        ...
        alert('Der er sket en fejl i besvarelsen');
    }
}
